...
#Matrix
FreeThrowsAttempts <- rbind(KobeBryant_FTA, JoeJohnson_FTA, LeBronJames_FTA,
...
plotting <- function(data, rows=1:10){
  toplot <- data[rows,,drop=FALSE]
  matplot(t(toplot), type = "b", pch=15:18, col = c(1:4,6))
  legend("bottomleft", inset=0.01, legend=Players[rows], col=c(1:4,6), pch=15:18, horiz=F)
}

plotting(FreeThrowsAttempts/Games)
plotting(FreeThrows/FreeThrowsAttempts)
plotting((Points-FreeThrows)/FieldGoals)
